...
def load(listbox):
    oschdir(filedialog.askdirectory(title='Open a songs directory'))
...
playlist = Listbox(list_box_frame, font=("Helvetica", 11), selectbackground='Gold')
...
resume_btn = Button(button_frame, text='Resume', bg='Aqua', font=("Georgia", 13),
                    width=7,
                    command=lambda: resume_song(song_status))
resume_btn.place(x=285, y=10)

load_btn = Button(button_frame, text='Load', bg='Aqua', font=("Georgia", 13),
                    width=35,
                    command=lambda: load(playlist))
load_btn.place(x=10, y=55)

Label(root, textvariable=song_status, bg='SteelBlue', font=("Times", 9),
